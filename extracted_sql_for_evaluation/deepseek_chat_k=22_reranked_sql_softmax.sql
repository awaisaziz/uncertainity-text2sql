...
SELECT T1.Stadium_ID ,  T1.Location ,  T1.Name ,  T1.Capacity ,  T1.Highest ,  T1.Lowest ,  T1.Average ,  count(T2.Stadium_ID) FROM stadium AS T1 ,  concert AS T2 WHERE T1.Stadium_ID  =  T2.Stadium_ID (+) GROUP BY T1.Stadium_ID ,  T1.Location ,  T1.Name ,  T1.Capacity ,  T1.Highest ,  T1.Lowest ,  T1.Average
...
SELECT T1.Name, T1.Capacity FROM stadium AS T1 WHERE T1.Stadium_ID = (SELECT T2.Stadium_ID FROM (SELECT Stadium_ID, count(*) FROM concert WHERE Year > 2013 GROUP BY Stadium_ID) AS T2 ORDER BY T2.count DESC LIMIT 1)
SELECT Year FROM (SELECT Year, count(*) FROM concert GROUP BY Year ORDER BY 2 DESC LIMIT 1) AS T1
SELECT Year FROM (SELECT Year ,  count(*) FROM concert GROUP BY Year ORDER BY count(*) DESC LIMIT 1) AS T1
...
SELECT Name ,  count(concert_ID) FROM singer_in_concert RIGHT JOIN singer ON singer_in_concert.Singer_ID  =  singer.Singer_ID GROUP BY Name
...
SELECT T1.Name ,  T1.Country FROM singer AS T1 WHERE T1.Song_Name ~ 'Hey'
...
SELECT T1.Name ,  T1.Location FROM stadium AS T1 WHERE T1.Stadium_ID IN (SELECT T2.Stadium_ID FROM concert AS T2 WHERE T2.Year  =  2014) AND T1.Stadium_ID IN (SELECT T2.Stadium_ID FROM concert AS T2 WHERE T2.Year  =  2015)
SELECT count(*) FROM concert WHERE Stadium_ID = (SELECT Stadium_ID FROM stadium HAVING Capacity = max(Capacity) LIMIT 1)
SELECT count(*) FROM concert WHERE Stadium_ID = (SELECT Stadium_ID FROM stadium HAVING Capacity = max(Capacity))
...
WITH MaxWeights AS (SELECT PetType, max(weight) AS max_weight FROM Pets GROUP BY PetType) SELECT PetType, max_weight FROM MaxWeights
...
SELECT PetType, avg(pet_age), max(pet_age) FROM Pets GROUP BY PetType, 1
...
SELECT ContId ,  Continent ,  count FROM (SELECT Continent ,  count(*) FROM countries GROUP BY Continent) AS T1 JOIN continents AS T2 ON T1.Continent  =  T2.Continent
SELECT ContId ,  Continent ,  (SELECT count(CountryId) FROM countries WHERE Continent  =  continents.Continent GROUP BY Continent) FROM continents
