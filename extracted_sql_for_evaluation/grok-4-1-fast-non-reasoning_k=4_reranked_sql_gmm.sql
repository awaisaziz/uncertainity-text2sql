...
SELECT Location, count(concert_ID) FROM stadium AS T1 JOIN concert AS T2 ON T1.Stadium_ID  =  T2.Stadium_ID GROUP BY Location
...
SELECT Name FROM stadium WHERE Stadium_ID NOT IN (SELECT DISTINCT Stadium_ID FROM concert WHERE Year = 2014)
SELECT concert_Name ,  Theme ,  count(Singer_ID) FROM concert JOIN singer_in_concert ON concert.concert_ID  =  singer_in_concert.concert_ID GROUP BY concert_ID ,  concert_Name ,  Theme
...
SELECT Name ,  (SELECT count(*) FROM singer_in_concert WHERE Singer_ID  =  s.Singer_ID) FROM singer AS s
...
SELECT Fname FROM Student S JOIN Has_Pet H ON S.StuID = H.StuID JOIN Pets P ON H.PetID = P.PetID WHERE P.PetType = 'dog' AND S.StuID NOT IN (SELECT StuID FROM Has_Pet JOIN Pets ON PetID = PetID WHERE PetType = 'cat')
...
SELECT PetID FROM Student AS T1 JOIN Has_Pet AS T2 ON T1.StuID  =  T2.StuID JOIN Pets AS T3 ON T2.PetID  =  T3.PetID WHERE T1.LName  =  'Smith'
SELECT H.StuID, count(H.PetID) FROM Has_Pet AS H WHERE H.StuID IN (SELECT StuID FROM Student) GROUP BY H.StuID
SELECT StuID, count(PetID) FROM Has_Pet JOIN Student ON Has_Pet.StuID  =  Student.StuID GROUP BY StuID
...
SELECT ContId, Continent, count(CountryId) FROM continents, countries WHERE continents.Continent  =  countries.Continent GROUP BY ContId, Continent
...
SELECT DISTINCT Maker FROM car_makers JOIN model_list ON car_makers.Id  =  model_list.Maker JOIN cars_data ON model_list.ModelId  =  cars_data.Id WHERE cars_data.Year  =  1970
