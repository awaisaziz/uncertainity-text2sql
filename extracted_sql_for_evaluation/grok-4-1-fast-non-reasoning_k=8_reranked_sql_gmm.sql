...
SELECT Location, (SELECT COUNT(*) FROM concert WHERE Stadium_ID  =  stadium.Stadium_ID) FROM stadium
...
SELECT DISTINCT S.Major, S.Age FROM Student S JOIN Has_Pet H ON S.StuID = H.StuID JOIN Pets P ON H.PetID = P.PetID WHERE P.PetType <> 'cat'
...
SELECT Model FROM (SELECT Model, Horsepower FROM cars_data C JOIN car_names CN ON C.Id  =  CN.MakeId ORDER BY Horsepower ASC LIMIT 1) AS t
SELECT Model FROM car_names WHERE MakeId IN (SELECT Id FROM cars_data WHERE Weight < (SELECT avg(Weight) FROM cars_data))
SELECT Model FROM car_names WHERE MakeId IN (SELECT DISTINCT MakeId FROM cars_data WHERE Weight < (SELECT AVG(Weight) FROM cars_data))
