...
SELECT avg(Capacity), max(Capacity) FROM stadium WHERE Capacity = Capacity
...
SELECT Year FROM concert GROUP BY Year HAVING count(*)  =  (SELECT cnt FROM (SELECT max(count(*)) AS cnt FROM concert GROUP BY Year) AS T26)
...
SELECT Name FROM stadium WHERE Stadium_ID NOT IN (SELECT Stadium_ID FROM concert WHERE Year = 2014)
SELECT concert_Name ,  Theme ,  (SELECT count(Singer_ID) FROM singer_in_concert AS T2 WHERE T2.concert_ID  =  concert.concert_ID GROUP BY T2.concert_ID) FROM concert
...
SELECT T1.Name ,  count(T2.concert_ID) FROM singer_in_concert AS T2 JOIN singer AS T1 ON T2.Singer_ID  =  T1.Singer_ID GROUP BY T1.Singer_ID ,  T1.Name
SELECT T1.Name FROM singer AS T1 WHERE T1.Singer_ID IN (SELECT T2.Singer_ID FROM singer_in_concert AS T2 WHERE T2.concert_ID IN (SELECT T3.concert_ID FROM concert AS T3 WHERE T3.Year = 2014) GROUP BY T2.Singer_ID)
