...
SELECT s.Location ,  s.Name FROM stadium s JOIN (SELECT Stadium_ID ,  COUNT(DISTINCT Year) as yr_cnt FROM concert WHERE Year IN (2014,2015) GROUP BY Stadium_ID HAVING yr_cnt = 2) c ON s.Stadium_ID  =  c.Stadium_ID
...
SELECT s1.Fname, s1.Age FROM Student s1, (SELECT DISTINCT StuID FROM Has_Pet h1, Pets p1 WHERE h1.PetID = p1.PetID AND p1.PetType = 'dog') d WHERE s1.StuID = d.StuID AND s1.StuID NOT IN (SELECT h2.StuID FROM Has_Pet h2, Pets p2 WHERE h2.PetID = p2.PetID AND p2.PetType = 'cat')
...
SELECT DISTINCT PetID FROM Has_Pet H JOIN Student S ON S.StuID = H.StuID WHERE S.LName = 'Smith'
