...
WITH stadium_concerts AS (SELECT Stadium_ID, count(*) AS concert_count FROM concert WHERE Year > 2013 GROUP BY Stadium_ID) SELECT T1.Name, T1.Capacity FROM stadium AS T1 WHERE T1.Stadium_ID = (SELECT Stadium_ID FROM stadium_concerts WHERE concert_count = (SELECT max(concert_count) FROM stadium_concerts))
