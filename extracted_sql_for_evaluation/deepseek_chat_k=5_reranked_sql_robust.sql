...
SELECT DISTINCT singer.Song_Name FROM singer WHERE singer.Age > (SELECT AVG(Age) FROM singer) ORDER BY singer.Song_Name;
...
SELECT singer.Name, COUNT(*) AS number_of_concerts FROM singer JOIN singer_in_concert ON singer.Singer_ID = singer_in_concert.Singer_ID GROUP BY singer.Singer_ID, singer.Name ORDER BY number_of_concerts DESC;
...
SELECT Name, Country FROM singer WHERE Song_Name LIKE '%Hey%';
SELECT DISTINCT stadium.Name, stadium.Location FROM stadium JOIN concert ON stadium.Stadium_ID = concert.Stadium_ID WHERE concert.Year = 2014 AND stadium.Stadium_ID IN ( SELECT Stadium_ID FROM concert WHERE Year = 2015 );
