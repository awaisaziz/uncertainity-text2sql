...
SELECT count(*) FROM concert WHERE Year IN (2014, 2015)
SELECT Name ,  count(concert_ID) FROM stadium LEFT JOIN concert ON stadium.Stadium_ID  =  concert.Stadium_ID GROUP BY stadium.Stadium_ID ,  Name
...
SELECT avg(Age) FROM Student WHERE StuID NOT IN (SELECT StuID FROM Has_Pet HAVING count(*) > 0)
SELECT count(DISTINCT Continent) FROM continents
SELECT count(Continent) FROM continents
SELECT ContId ,  Continent ,  count FROM continents AS T1 JOIN (SELECT Continent ,  count(CountryId) FROM countries GROUP BY Continent) AS T2 ON T1.Continent  =  T2.Continent
...
SELECT T1.Model FROM model_list AS T1 JOIN car_names AS T2 ON T1.Model = T2.Model JOIN cars_data AS T3 ON T2.MakeId = T3.Id GROUP BY T1.Model HAVING T3.Horsepower = (SELECT min(Horsepower) FROM cars_data)
